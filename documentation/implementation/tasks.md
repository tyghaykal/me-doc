...
## Phase 17: White-label branding, offline/local mode, DB backup cron, AI slash commands

Plan: `documentation/implementation/whitelabel-offline-backup-ai-plan.md`. Four independent features, implemented in parallel.

- [x] **#108** White-label product name — `PRODUCT_NAME` env var (default `MeDoc`) added to `config.rs`, threaded through `EmailClient` (OTP + share-notification subjects/HTML templates). Frontend: `nuxt.config.ts` exposes `runtimeConfig.public.productName` from `NUXT_PUBLIC_PRODUCT_NAME` (wired in `docker-compose.yml` from `PRODUCT_NAME`, matching the existing `NUXT_PUBLIC_MINIO_BASE` derivation pattern — not duplicated into `.env`/`.env.example`); `app.vue` sets the document title from it at runtime (not `nuxt.config.ts`'s static `app.head.title`, which is frozen at image build time and would miss a runtime-only override). `LandingNav.vue`/`LandingFooter.vue` read it too. Verified live: overrode `PRODUCT_NAME=Acme Docs` in `.env`, recreated `backend`+`frontend` containers with no rebuild, confirmed both the rendered `<title>` and the backend's own env reflected the new name, then reverted.
- [x] **#109** Offline/local mode — new `/app/local` route (`pages/app/local.vue`). `Editor.vue`'s non-collab extensions (StarterKit, Table, Image, the custom Diagram/comment/definition-list nodes, SlashCommand, etc.) were extracted into `composables/useEditorExtensions.ts`, shared by `Editor.vue` (layers Collaboration/CollaborationCaret on top) and the new `LocalEditor.vue` (plain `useEditor()`, no Y.Doc, no websocket, images stored as data URLs since there's no server to upload attachments to). `local.vue` never renders `AppTopbar.vue`'s Share button, so there's no Share control anywhere in local mode. Sidebar entry point added in `AppSidebar.vue`.
  - **Follow-up**: switched the saved file format from a standalone `.html` document to plain Markdown by default (`.html`/`.htm` still opens via the old raw-HTML path, so a file saved before this change doesn't come back garbled). New `utils/proseMirrorMarkdown.ts` walks `editor.getJSON()` and mirrors `backend/src/export/mod.rs`'s `yjs_to_markdown` block-for-block (same heading/list/table/code-fence/mark conventions, including reproducing its exact nested-list blank-line artifact rather than "fixing" it) so a locally-saved file reads identically to one exported from a server page — verified against a hand-built ProseMirror doc exercising every node/mark type. Loading a `.md` file reuses the same `markdownToHtml` conversion the existing server-backed `.md` import already uses. Also reworked `local.vue`'s whole page template to match `[[pageId]].vue`'s layout exactly (`AppSidebar` + a topbar with the same structure/button styling as `AppTopbar.vue`, the same big icon+title header Editor.vue shows, the same `TableOfContents`) instead of a bespoke minimal toolbar — `PAGE_ICON_CHOICES` extracted from `Editor.vue` into `stores/pages.ts` so both share one icon set. Title/icon are local-only fields (no server row to hold them): title round-trips losslessly through the filename (no slugifying, since a local file has no URL-safety constraint), icon is cosmetic-only and resets on Open/New since a plain `.md` file has no side channel to carry it. Verified with a Playwright screenshot comparison against a real online document — sidebar, topbar structure, and content-area layout match; only the intentional differences show (Private/Share → Offline, Export/Share → Save/Save As).
  - **Follow-up**: autosave + a "Local" sidebar section listing previously-saved local documents. New `composables/useLocalDocs.ts` — the visible list (`{name, savedAt}`) lives in `localStorage` (same `useState`-backed, write-through pattern as `useRecents`, including its dev-mode `console.assert` self-check), but the actual `FileSystemFileHandle` can only be persisted via IndexedDB (the sole client storage with structured-clone support for that host object type — cookies/localStorage can only hold strings). `local.vue`: autosave has two triggers — a 1.5s debounce after the user stops typing (which in practice fires right after finishing a line, since each keystroke keeps pushing it back) and a 20s fixed-interval fallback for a long unbroken typing stretch that keeps rescheduling the debounce — both silent no-ops until a handle exists (Open or Save As), since a browser can't silently pop the save-file picker without a user gesture, so there's no such thing as a silent *first* save. New `SidebarLocalDocs.vue` (heading text "Local", mirroring `SidebarRecents.vue`'s exact structure) renders the list; clicking an entry navigates to `/app/local?open=<name>`, which resolves the handle from IndexedDB, re-grants permission (the click itself is the required user gesture), and drops the entry with a clear error if the handle's gone or access was denied. Verified: real Chromium OS file pickers aren't Playwright-automatable, so `local.spec.ts` mocks `showSaveFilePicker`/`showOpenFilePicker` with an in-memory handle via `addInitScript` — the real `createWritable`/`write`/`close` and debounce-scheduling code runs unmocked, confirming Save As establishes a handle, an unrelated edit autosaves on its own within 5s with no Save click, and the sidebar picks the entry up live; a second test seeds a `localStorage` entry with no matching IndexedDB handle and confirms the stale-entry error + list cleanup. Full 25-test suite passes (two more pre-existing-flaky, unrelated failures reproduced clean under `--workers=1`).
