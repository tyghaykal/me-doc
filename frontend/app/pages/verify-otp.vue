...
async function submit() {
  ...
  loading.value = true
  try {
    await auth.verifyRegister(email, code.value)
...
<template>
  <main class="min-h-screen flex items-center justify-center p-8 font-sans bg-neutral-50 dark:bg-neutral-950">
    ...
    >
      <h1 class="text-3xl font-bold text-neutral-900 dark:text-neutral-100">Verify your email</h1>
      <p class="mt-2 text-neutral-600 dark:text-neutral-400">
        Enter the 6-digit code sent to
        <span class="font-medium text-neutral-900 dark:text-neutral-100">{{ email || 'your email' }}</span>.
      </p>

      <label for="verify-otp-code" class="block mt-6 text-sm font-medium text-neutral-700 dark:text-neutral-300">Code</label>
      <input
        id="verify-otp-code"
        v-model="code"
        inputmode="numeric"
